...
class DashboardDescription extends Component {
  ...
  componentWillMount() {
    ...
    if (identity.ID_GROUP === 5) {
      this.setState({
        cardStyle: { backgroundColor: 'rgb(0,0,0,0)', border: 'none', color: '#fff' },
        style: { backgroundColor: 'rgb(0,0,0,0.8)', border: 'none', color: '#fff', fontSize: '18px', textAlign: 'center' },
  ...
  componentDidMount() {
    ...
    if (identity.branch_code === 'PUSAT') {
      ...
      let dataLastUpdate = dashboardAction.getLastUpdate();
      dataLastUpdate.then((res) => {
        if (res.length > 0) {
          this.setState({
            lastUpdate: res[0],
          });
  ...
  render() {
    const { percentage, bagiHasilSettlement, bankPercentagePerRuas, monthlyByRealisasi } = this.props
    ...
    if (percentage.percentageRuas.length > 1) {
      var table0 = percentage.percentageRuas.map((val, index) => {
        let yearReal = val.TRANSACTION_REALISASI_YEARLY
        let yearPlan = val.TRANSACTION_RENCANA_YEARLY
        let yearPercentage = val.TRANSACTION_YEARLY_PERCENTAGE.toFixed(2)
        let yearRealLalin = val.TRAFFIC_REALISASI_YEARLY
        let yearPlanLalin = val.TRAFFIC_RENCANA_YEARLY
        let yearPercentagLalin = val.TRAFFIC_YEARLY_PERCENTAGE.toFixed(2)
        let monthReal = val.TRANSACTION_REALISASI_MONTHLY
        let monthPlan = val.TRANSACTION_RENCANA_MONTHLY
        let monthPercentage = val.TRANSACTION_MONTHLY_PERCENTAGE.toFixed(2)
        let monthRealLalin = val.TRAFFIC_REALISASI_MONTHLY
        let monthPlanLalin = val.TRAFFIC_RENCANA_MONTHLY
        let monthPercentageLalin = val.TRAFFIC_MONTHLY_PERCENTAGE.toFixed(2)
    ...
    var table = tableList.map((val, index) => {
      ...
      let monthPlan = (val.rencana[val.rencana.length - 1]).toString()
        ...
        <Row>
          <Col xs="12" lg="12">
            <Card style={this.state.cardStyle}>
              <CardHeader style={this.state.cardStyle}>
              ...
              <CardBody>
                <Row>
                  <Col>
                    <p><b>Data Pendapatan Seluruh Ruas Tol Bagi Hasil HK </b>
                      <small> (Data update per minggu)</small>
                      <small className="float-right"> Last Update {this.state.lastUpdate.LAST_BAGI_HASIL} </small>
                    </p>
                  </Col>
                </Row>
                <Row>
                  <Col>
                    <Table striped responsive bordered style={this.state.style}>
                ...
                <Row>
                  <Col>
                    <p><b>Data Pendapatan Seluruh Ruas HK non Bagi Hasil</b>
                      <small> (Data update per shift)</small>
                      <small className="float-right"> Last Update {this.state.lastUpdate.LAST_LATOL} Shift-{this.state.lastUpdate.SHIFT} </small>
                    </p>
                  </Col>
                </Row>
                <Row>
                ...
                <Row>
                  <Col>
                    <p><b>Pencapaian Settlement per Bank</b>
                      <small> (Data update per minggu)</small>
                      <small className="float-right"> Last Update {this.state.lastUpdate.LAST_REKENING_KORAN} </small>
                    </p>
                  </Col>
                </Row>
                <Row>
                    ...
                    <Table striped responsive bordered style={this.state.style}>
                      <thead style={{ fontSize: 11 + 'px' }}>
                        <tr>
                          ...
                          <th style={{ textAlign: "center" }} colSpan="4">Bank</th>
                        </tr>
                        <tr>
